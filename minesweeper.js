      ...
      if (board[row][col] !== 'mine') {
        ...
        for (let i = -1; i <= 1; i++) {
          for (let j = -1; j <= 1; j++) {
            const r = row + i;
            const c = col + j;
            if (r >= 0 && r < NUM_ROWS && c >= 0 && c < NUM_COLS && board[r][c] === 'mine') {
              numMines++;
            }
          }
        }
    ...
    if (board[row][col] == 0) {
        for (let r = -1; r <= 1; ++r) {
            for (let c = -1; c <= 1; ++c) {
                const temp_r = row + r;
                const temp_c = col + c;
                if (temp_r >= 0 && temp_r < NUM_ROWS && temp_c >=0 && temp_c < NUM_COLS  && cell_status[temp_r][temp_c] == false) revealCell(temp_r, temp_c);
            }
        }
    } 
    ...
    for (let row = 0; row < NUM_ROWS; row++) {
      for (let col = 0; col < NUM_COLS; col++) {
        ...
        if (isCellOpened){
            ...
            context.fillText(cell.toString(), col * CELL_SIZE + CELL_SIZE / 3, row * CELL_SIZE + CELL_SIZE * 2 / 3);
            }
        }
        else {
            if (isCellFlagged) {
                context.fillStyle = 'black';
                context.font = 'bold 16px Arial';
                context.fillText('F', col * CELL_SIZE + CELL_SIZE / 3, row * CELL_SIZE + CELL_SIZE * 2 / 3);
            }
            else {
                context.fillStyle = 'white';
                context.fillRect(col * CELL_SIZE, row * CELL_SIZE, CELL_SIZE, CELL_SIZE);
                context.fillStyle = 'black';
                context.fillRect(col * CELL_SIZE, row * CELL_SIZE, CELL_SIZE - 2, CELL_SIZE - 2);
            }
        }
      }
    }
